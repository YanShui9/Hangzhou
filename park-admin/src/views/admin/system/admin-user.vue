<template>
  <div class="admin-user-page page-list-flex">
    ...
    <el-dialog title="批量导入管理员账号" :visible.sync="importVisible" width="500px" :close-on-click-modal="false">
      <el-upload
        class="import-upload"
        drag
        action=""
...
export default {
  ...
  methods: {
    ...
    handleDownloadTemplate() {
      downloadAdminUserTemplate().then(blob => {
        const url = window.URL.createObjectURL(blob)
        const link = document.createElement('a')
        link.href = url
        link.download = '管理员账号导入模板.xlsx'
