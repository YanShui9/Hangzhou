          ...
          <el-form ref="loginForm" :model="loginForm" :rules="loginRules">
            ...
            <div class="form-group">
              ...
              <div class="captcha-row">
                <el-input v-model="loginForm.captchaCode" placeholder="请输入验证码" @keyup.enter.native="handleLogin" />
...
export default {
  ...
  data() {
    return {
      loginForm: { username: '', password: '', roleType: 1, captchaKey: '', captchaCode: '' },
      loginRules: {
      ...
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          ...
          this.$store.dispatch('user/login', this.loginForm)
            .then(() => { this.$router.push({ path: '/' }); this.$message.success('登录成功') })
            .catch(() => {
...
.captcha-img { height: 38px; cursor: pointer; border-radius: 4px; border: 1px solid #DCDFE6; flex-shrink: 0; }
.captcha-placeholder { height: 38px; width: 120px; display: flex; align-items: center; justify-content: center; font-size: 13px; color: #909399; border: 1px dashed #DCDFE6; border-radius: 4px; cursor: pointer; flex-shrink: 0; }
.btn-login {
