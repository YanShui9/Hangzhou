...
public class IndustryDevelopmentParseService {
    ...
    private EnterpriseInfo convertToEntity(IndustryDevelopmentExcelData data, Map<String, Long> parkNameToIdMap) {
        ...
        enterprise.setRegisteredAddress(data.getRegisteredAddress());
        enterprise.setHonor(data.getHonor());
        ...
        enterprise.setRegisterStatus(data.getRegisterStatus());
        ...
        if (data.getAnnualOutput() != null && !data.getAnnualOutput().trim().isEmpty()) {
            try {
                enterprise.setAnnualOutput(new BigDecimal(data.getAnnualOutput().trim()));
            } catch (NumberFormatException e) {
                enterprise.setAnnualOutput(BigDecimal.ZERO);
            }
        }
        
        if (data.getAnnualTax() != null && !data.getAnnualTax().trim().isEmpty()) {
            try {
                enterprise.setAnnualTax(new BigDecimal(data.getAnnualTax().trim()));
            } catch (NumberFormatException e) {
                enterprise.setAnnualTax(BigDecimal.ZERO);
            }
        }
        ...
        if (data.getSettledStartTime() != null && !data.getSettledStartTime().trim().isEmpty()) {
            try {
                enterprise.setEntryStartTime(LocalDate.parse(data.getSettledStartTime().trim(), DATE_FORMATTER));
            } catch (DateTimeParseException e) {
                enterprise.setEntryStartTime(null);
            }
        }
        
        if (data.getSettledEndTime() != null && !data.getSettledEndTime().trim().isEmpty()) {
            try {
                enterprise.setEntryEndTime(LocalDate.parse(data.getSettledEndTime().trim(), DATE_FORMATTER));
            } catch (DateTimeParseException e) {
                enterprise.setEntryEndTime(null);
            }
        }
